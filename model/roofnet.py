...
class RoofNet(nn.Module):
    def __init__(self, model_cfg, color=False, nir=False, intensity=False):
        super().__init__()
        self.use_edge = False
        self.model_cfg = model_cfg
        self.input_channel = 3 + (3 if color else 0) + (1 if nir else 0) + (1 if intensity else 0)
        self.keypoint_det_net = PointNet2(model_cfg.PointNet2, self.input_channel, color, nir, intensity)
