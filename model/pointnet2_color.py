...
def save_colored_point_cloud(xyz, cls, output_dir, batch_size=64):
    """
    将每个点云根据分类结果设置颜色，保存为.ply文件

    :param xyz: [batch_size, n, 3] 点云数据
    :param cls: [batch_size, n] 分类标签 (0或1)
    :param output_dir: 输出文件夹路径
    :param batch_size: 点云的数量 (默认64)
    """
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    for i in range(batch_size):
        points = xyz[i].cpu().numpy()  # 获取当前点云的点坐标
        labels = cls[i].cpu().numpy()  # 获取当前点云的分类标签
        
        # 创建颜色数组，红色表示目标点，白色表示非目标点
        colors = np.zeros((points.shape[0], 3))  # 默认全为白色
        colors[labels == 1] = [1, 0, 0]  # 分类为1的点设置为红色
        ...
        # 保存为.ply文件
        output_file = os.path.join(output_dir, f"point_cloud_{i}.ply")
...
class PointNet2(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        if self.training:
            ...
            # offset, cls = self.assign_targets(xyz, vectors, self.model_cfg.KnnPoint)
        ...
        '''
        # if self.f:
        #     save_path = "/data/haoran/Point2Roof/assign_target_building3d_train_knn_2048"
    ...
    # def assign_targets(self, points, gvs, npoint):
    #     # idx: torch.Size([64, 43, 6])
    #     # idx_add: torch.Size([64, 6])
    #     dist2, idx = knn_query(npoint, points, gvs)
    #     batch_size = gvs.size()[0]
    #     idx_add = torch.arange(batch_size).to(idx.device).unsqueeze(-1).repeat(1, idx.shape[-1]) * gvs.shape[1]
    #     gvs = gvs.view(-1, 3)
    #     idx_add += idx
    #     target_points = gvs[idx_add.view(-1)].view(batch_size, -1, 3)  # 获取目标点对应位置的点

    #     dis = target_points - points  # 计算目标点和当前点的差值
    #     dis /= dist2.sqrt()  # 用距离进行归一化

    #     # 生成标签：如果找到邻居，则为1，否则为0
    #     label = torch.where(idx >= 0, torch.ones(idx.shape).to(idx.device),
    #                         torch.zeros(idx.shape).to(idx.device))
    #     return dis, label
